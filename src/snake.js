  ...
  render: function() {
    ...
    return (
        React.DOM.div( {className:"snake-game"}, [
          React.DOM.h1( {className:"snake-score"}, ["Current length: ", this.state.snake.length]),
          React.DOM.div(
              {ref:"board",
...
function updateHighScore(snake_length){
  if(parseInt(snake_length)>window.localStorage.getItem("highscore")){
    alert("New highscore!");
    window.localStorage.setItem("highscore",parseInt(snake_length))
  }
}
...
React.renderComponent(SnakeGame(null, null ), document.getElementById("root"));
